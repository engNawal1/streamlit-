...
def style_fig(fig, height=380):
    fig.update_layout(
        ...
        font=dict(family="Tajawal, sans-serif", color="#1A1F2B", size=13),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        margin=dict(l=10, r=10, t=45, b=10),
        height=height,
        legend=dict(orientation="h", y=-0.18, font=dict(color="#1A1F2B", size=12)),
    )
    fig.update_xaxes(
        tickfont=dict(color="#1A1F2B", size=12),
        title_font=dict(color="#1A1F2B", size=13),
        gridcolor="#E7EAF3", linecolor="#C7CBDA",
    )
    fig.update_yaxes(
        tickfont=dict(color="#1A1F2B", size=12),
        title_font=dict(color="#1A1F2B", size=13),
